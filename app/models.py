...
class User(UserMixin, db.Model):
    ...
    def borrow_book(self, book):
        if self.logs.filter(Log.returned == 0, Log.return_timestamp < datetime.now()).count() > 0:
            return False, u"Unable to borrow, you have overdue books not returned!"
        if self.borrowing(book):
            return False, u'It looks like you have already borrowed this book!'
        if not book.can_borrow():
            return False, u'This book is too hot, we no longer have a collection, please wait for someone to return it and borrow it again!'

        db.session.add(Log(self, book))
        return True, u'You have successfully get a copy of %s' % book.title
