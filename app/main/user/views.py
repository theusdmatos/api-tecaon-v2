...
def index():
    ...
    return render_template("user.html", users=users, pagination=pagination, title=u"Usuários registrados")
...
def edit(user_id):
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(u'Perfil atualizado com sucesso!', "info")
        ...
        return render_template('user_edit.html', form=form, user=the_user, title=u"Editar Perfil")
    else:
        abort(403)
    ...
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        ...
        if avatar_upload_form.validate_on_submit():
            if 'avatar' in request.files:
                ...
                db.session.add(the_user)
                db.session.commit()
                flash(u'Imagem do perfil atualizada com sucesso!', 'success')
                return redirect(url_for('user.detail', user_id=user_id))
        ...
        return render_template('avatar_edit.html', user=the_user, avatar_edit_form=avatar_edit_form,
                               avatar_upload_form=avatar_upload_form, title=u"Alterar foto do perfil")
    else:
        abort(403)
