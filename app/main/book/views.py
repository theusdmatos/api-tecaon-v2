...
def edit(book_id):
    ...
    if form.validate_on_submit():
       ...
       # book.pubdate = form.pubdate.data
        ...
        db.session.add(book)
        db.session.commit()
        flash(u'Informações do livro salvas!', 'sucess')
  ...
  #  form.price.data = book.price
   ...
   # form.catalog.data = book.catalog or ""
    return render_template("book_edit.html", form=form, book=book, title=u"Editar informações do livro")
...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'O livro %s foi adicionado à biblioteca!' % new_book.title, 'success')
        return redirect(url_for('book.detail', book_id=new_book.id))
    return render_template("book_edit.html", form=form, title=u"Adicionar novo livro")
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'O livro foi excluído com sucesso, os usuários não podem mais vê-lo', 'info')
...
def put_back(book_id):
    ...
    db.session.add(the_book)
    db.session.commit()
    flash(u'Restaurado com sucesso o livro, o usuário agora pode vê-lo', 'info')
    return redirect(request.args.get('next') or url_for('book.detail', book_id=book_id))
